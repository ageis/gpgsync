...
class GnuPG(object):
    def __init__(self, common):
        self.common = common
        self.appdata_path = self.common.settings.get_appdata_path()

        self.homedir = tempfile.mkdtemp()
        self.common.log('GnuPG', '__init__: created homedir: {}'.format(self.homedir))
    ...
    def __del__(self):
        ...
        self.common.log('GnuPG', '__del__: deleted homedir: {}'.format(self.homedir))
    ...
    def recv_key(self, keyserver, fp, use_proxy, proxy_host, proxy_port):
        self.common.log('GnuPG', "recv_key: keyserver={}, fp={}, use_proxy={}, proxy_host={}, proxy_port={}".format(keyserver, fp, use_proxy, proxy_host, proxy_port))
    ...
    def export_pubkey_to_disk(self, fp):
        ...
        self.common.log('GnuPG', "export_pubkey_to_disk: fp={}".format(fp))

        if not self.appdata_path:
            self.common.log('GnuPG', "export_pubkey_to_disk: appdata_path not set, skipping")
            return
    ...
    def import_pubkey_from_disk(self, fp):
        ...
        self.common.log('GnuPG', "import_pubkey_from_disk: fp={}".format(fp))

        if not self.appdata_path:
            self.common.log('GnuPG', "import_pubkey_from_disk: appdata_path not set, skipping")
            return
    ...
    def delete_pubkey_from_disk(self, fp):
        ...
        self.common.log('GnuPG', "delete_pubkey_from_disk: fp={}".format(fp))

        if not self.appdata_path:
            self.common.log('GnuPG', "delete_pubkey_from_disk: appdata_path not set, skipping")
            return
    ...
    def test_key(self, fp):
        self.common.log('GnuPG', "test_key: fp={}".format(fp))
    ...
    def get_uid(self, fp):
        self.common.log('GnuPG', "get_uid: fp={}".format(fp))
    ...
    def verify(self, msg_sig, msg, fp):
        self.common.log('GnuPG', "verify: (not displaying msg_sig, msg), fp={}".format(fp))
    ...
    def list_all_keyids(self, fp):
        self.common.log('GnuPG', "list_all_keyids: fp={}".format(fp))
    ...
    def import_to_default_homedir(self, fp):
        self.common.log('GnuPG', "import_to_default_homedir: fp={}".format(fp))
        ...
        if not b'gpg: WARNING: nothing exported' in err:
            ...
            if out != '':
                self.common.log('GnuPG', 'import_to_default_homedir: stdout: {}'.format(out))
            if err != '':
                self.common.log('GnuPG', 'import_to_default_homedir: stderr: {}'.format(err))

    def _gpg(self, args, input=None):
        default_args = [self.gpg_path, '--batch', '--no-tty', '--homedir', self.homedir]

        self.common.log('GnuPG', '_gpg: args: {}'.format(default_args + args))
        ...
        if out != '':
            self.common.log('GnuPG', '_gpg: stdout: {})'.format(out))
        if err != '':
            self.common.log('GnuPG', '_gpg: stderr: {}'.format(err))
        return out, err
